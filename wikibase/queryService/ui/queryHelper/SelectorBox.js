			...
			createTags = function () {
				...
				return self._searchEntitiesSparql( null, entity, triple, sparql  ).then(  function ( d ) {
					d.forEach( function( t ) {
						tags.push( {
							text: t.text,
							weight:  ( Math.round(Math.random() * 10) > 9 ? 5 : 2  ),
							link: '#',
							html: {
								title: t.description,
								'data-id': t.id
							...
							handlers: {
								click: function ( e ) {
									...
									if( t.data.propertyId ) {
										listener( t.id, t.text, t.data.propertyId, updateCallback  );
										return false;
									}

									self._suggestPropertyId( t.id ).always( function ( propertyId ) {
										listener( t.id, t.text, propertyId, updateCallback  );
									} );
									return false;
								}
							}
